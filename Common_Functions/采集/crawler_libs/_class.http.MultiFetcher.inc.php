...
class MultiFetcher{
	 ...
	 * @return mixed[]
	 */
	public function fetch($headers=array()){
		...
		foreach ($url_array as  $i => $url_item)
		{
			...
			curl_setopt( $conn[$i], CURLOPT_USERAGENT,'Mozilla/5.0 (Windows; U; Windows NT 5.1; zh-CN; rv:1.9.0.17) Firefox/3.0.17 GTB6' );
			if($this->post){
				curl_setopt($conn[$i], CURLOPT_POST, 1);
				curl_setopt($conn[$i], CURLOPT_POSTFIELDS, $this->post_fields);
			}
			if($this->auto_referer){
				curl_setopt($conn[$i], CURLOPT_REFERER, $url_item['url']);
			}
			...
			$http_header[] 	= 'Accept: *'.'/*';
//			$http_header[] 	= 'Cookie: __utma=15115312.198709730611041570.1238064510.1242362852.1242366746.8; __utmz=15115312.1238064510.1.1.utmcsr=(direct)|utmccn=(direct)|utmcmd=(none); __utmb=15115312.6.10.1242366746; __utmc=15115312; cookieLangId=en_us; armory.cookieSearch=jim; JSESSIONID=ABC79E5E577CB283ABA9C64F5ECF266E.app22_07';
			$http_header[] 	= "Accept-Language: zh-cn";
			$http_header = array_merge($http_header, $headers);
...
// 下面注释部分代码查了半天没看出问题。在windows下运行超时。故去掉 # taozy.wu
//			do{
