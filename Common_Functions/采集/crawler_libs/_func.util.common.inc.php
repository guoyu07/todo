...
function trim_params(&$params, $function=null, $remove_null=false, $remove_empty=false){
	if(!$function) $function = "trim";
	foreach($params as $key=>&$param){
		if($param===null){
			if($remove_null) unset($params[$key]);
		}if(!$param){
			if($remove_empty) unset($params[$key]);
		}else{
...
function array_uunique($input, $callback){
	...
	while(true){
		...
		if($m>=count($input)) break;
		for($n=count($input)-1; $n>=$m+1; $n--){
			$flag = call_user_func_array($callback, array_merge(array($input[$m], $input[$n]), $arguments));	
			if($flag == 1){
				unset($input[$n]);
			}
		}
...
function array_column2row(&$rows, $field){
        $ret = array();
        foreach($rows as $row){
...
function array_row2kv(&$rows, $key, $value){
        $ret = array();
        foreach($rows as $row){
...
function array_row2combie(&$rows, $key){
        $ret = array();
        foreach($rows as $row){
                if(!$ret[$row[$key]]){
                        $ret[$row[$key]] = array();
                }
                array_push($ret[$row[$key]], $row);
        }
        return $ret;
}

function array_row2key(&$rows, $key){
	$ret = array();
	foreach($rows as $row){
		$ret[$row[$key]] = $row;
	}
